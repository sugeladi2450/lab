...
struct ggml_kompute_context {
	...
	ggml_kompute_context(int device) :
		device(device),
		name(ggml_kompute_format_name(device)) {}
};
...
#ifdef __linux__
__attribute__((constructor)) static void enable_sam() {
	setenv("RADV_PERFTEST", "sam", false);
}
#endif

static bool ggml_vk_checkPhysicalDeviceFeatures(
	...
	if (!availableFeatures12.storageBuffer8BitAccess ||
		...
		!availableFeatures12.shaderFloat16 || !availableFeatures12.shaderInt8) {
		return false;
	}
...
static std::vector<ggml_vk_device> ggml_vk_available_devices_internal(
	...
	for (uint32_t i = 0; i < deviceCount; i++) {
		...
		if (has_maintenance4) {
			d.maxAlloc = std::min(
				dev_props3.maxMemoryAllocationSize, dev_props4.maxBufferSize
			);
		} else {
	...
	std::stable_sort(
		...
		[](const ggml_vk_device &lhs, const ggml_vk_device &rhs) -> bool {
			if (lhs.type != rhs.type) {
				if (lhs.type == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
					return true;
				if (rhs.type == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
					return false;

				if (lhs.type == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU)
					return true;
				if (rhs.type == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU)
					return false;
			}
...
ggml_vk_device *ggml_vk_available_devices(
	size_t memoryRequired, size_t *count
) {
...
static void ggml_vk_filterByVendor(
	std::vector<ggml_vk_device> &devices, const std::string &targetVendor
) {
	devices.erase(
		std::remove_if(
			devices.begin(),
...
static void ggml_vk_filterByName(
	std::vector<ggml_vk_device> &devices, const std::string &targetName
) {
	devices.erase(
		std::remove_if(
			devices.begin(),
...
static bool ggml_vk_get_device(
	ggml_vk_device *device, size_t memoryRequired, const std::string &name
) {
	if (name.empty())
		return false;
...
bool ggml_vk_get_device(
	ggml_vk_device *device, size_t memoryRequired, const char *name
) {
...
ggml_vk_device ggml_vk_current_device() {
	...
	ggml_vk_filterByName(
		devices,
		komputeManager()->physicalDevice()->getProperties().deviceName.data()
	);
	GGML_ASSERT(!devices.empty());
	return devices.front();
}

static void ggml_vk_allocate_descriptor_pool(
	struct ggml_kompute_context *ctx, size_t size
) {
...
static void ggml_vk_free_descriptor_pool(struct ggml_kompute_context *ctx) {
	if (ctx->pool) {
		komputeManager()->device()->destroy(
			*ctx->pool, (vk::Optional<const vk::AllocationCallbacks>)nullptr
		);
		ctx->pool = nullptr;
	}
}

static vk::Buffer *ggml_vk_allocate_buffer(size_t size) {
	...
	vk::Result r		 = komputeManager()->device()->createBuffer(
		&bufferCreateInfo, nullptr, vkBuffer
	);
	if (r != vk::Result::eSuccess)
...
static vk::DeviceMemory *ggml_vk_allocate(
	size_t size,
	...
	vk::PhysicalDeviceMemoryProperties memoryProperties =
		komputeManager()->physicalDevice()->getMemoryProperties();
	for (uint32_t i = 0; i < memoryProperties.memoryTypeCount; i++) {
		const vk::MemoryType &memoryType = memoryProperties.memoryTypes[i];
		const vk::MemoryHeap &memoryHeap =
	...
	vk::Result r = komputeManager()->device()->allocateMemory(
		&allocInfo, nullptr, vkDeviceMemory
	);
	if (r != vk::Result::eSuccess) {
...
static size_t ggml_vk_aligned_offset(
	ggml_backend_buffer_t buffer, size_t offset
) {
	...
	return (offset / minStorageBufferOffsetAlignment) *
...
static ggml_vk_memory ggml_vk_allocate(size_t size) {
	...
	bool isHostVisible = false;
	{
		...
		memory.primaryMemory = ggml_vk_allocate(
			size, memoryPropertyFlags, memoryRequirements, &isHostVisible
		);
		komputeManager()->device()->bindBufferMemory(
			*memory.primaryBuffer, *memory.primaryMemory, 0
		);
		if (isHostVisible) {
			vk::Result r = komputeManager()->device()->mapMemory(
	...
	if (!isHostVisible) {
		...
		memory.stagingMemory = ggml_vk_allocate(
			size, memoryPropertyFlags, memoryRequirements, &isHostVisible
		);
		komputeManager()->device()->bindBufferMemory(
			*memory.stagingBuffer, *memory.stagingMemory, 0
		);
		vk::Result r = komputeManager()->device()->mapMemory(
			*memory.stagingMemory, 0, size, vk::MemoryMapFlags(), &memory.data
		);
		if (r != vk::Result::eSuccess)
			std::cerr << "Error mapping memory" << vk::to_string(r);
	}
...
static const char *ggml_backend_kompute_buffer_type_get_name(
	ggml_backend_buffer_type_t buft
);

static ggml_vk_memory *ggml_vk_find_tensor(
	const struct ggml_tensor *t, uint64_t &offset
) {
	...
	GGML_ASSERT(
		buffer && buffer->buft->iface.get_name ==
	...
	GGML_ASSERT(
		ioffs >= 0 && ioffs + int64_t(ggml_nbytes(t)) <= int64_t(buffer->size)
	);
...
static const std::shared_ptr<kp::Tensor> ggml_vk_get_tensor(
	const struct ggml_tensor *t, uint32_t *alignedOffset = nullptr
) {
...
static std::vector<uint32_t> getSpirvShader(
...
inline static uint32_t safe_divide(uint32_t a, uint32_t b) {
	if (b <= 1) {
		return a;
	}
	if ((a % b) != 0) {
...
static void ggml_vk_add(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_add_comp_spv, kp::shader_data::op_add_comp_spv_len
	);
	...
	struct PushConstants {
		...
		safe_divide(inBOff, 4),
		safe_divide(outOff, 4),
		ne00,
		nb00,
		nb01,
		nb02,
	...
	if (!komputeManager()->hasAlgorithm(__func__)) {
		s_algo = komputeManager()->algorithm<float, PushConstants>(
			__func__,
			s_kompute_context->pool.get(),
			{inA, inB, out},
			spirv,
			{unsigned(ne01), unsigned(ne02), unsigned(ne03)},
			{},
			{pushConsts}
		);
	} else {
...
static void ggml_vk_addrow(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_addrow_comp_spv,
...
static void ggml_vk_mul(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_comp_spv, kp::shader_data::op_mul_comp_spv_len
	);
...
static void ggml_vk_scale(
	...
	const static auto spirv_1 = getSpirvShader(
		kp::shader_data::op_scale_comp_spv,
		kp::shader_data::op_scale_comp_spv_len
	);
	const static auto spirv_8 = getSpirvShader(
		kp::shader_data::op_scale_8_comp_spv,
		kp::shader_data::op_scale_8_comp_spv_len
	);

	struct PushConstants {
		uint32_t inOff, outOff;
		float scale;
	} const pushConsts{safe_divide(inOff, 4), safe_divide(outOff, 4), scale};
...
static void ggml_vk_silu(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_silu_comp_spv, kp::shader_data::op_silu_comp_spv_len
	);
...
static void ggml_vk_relu(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_relu_comp_spv, kp::shader_data::op_relu_comp_spv_len
	);
...
static void ggml_vk_gelu(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_gelu_comp_spv, kp::shader_data::op_gelu_comp_spv_len
	);
...
static void ggml_vk_soft_max(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_softmax_comp_spv,
	...
	if (!komputeManager()->hasAlgorithm(__func__)) {
		...
		s_algo = komputeManager()->algorithm<uint32_t, PushConstants>(
			__func__,
			s_kompute_context->pool.get(),
			{inA, inB_, out},
			spirv,
...
static void ggml_vk_norm(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_norm_comp_spv, kp::shader_data::op_norm_comp_spv_len
	);
...
static void ggml_vk_rms_norm(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_rmsnorm_comp_spv,
...
static void ggml_vk_diag_mask_inf(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_diagmask_comp_spv,
	...
	struct PushConstants {
		...
		safe_divide(inOff, 4), safe_divide(outOff, 4), n_past, ne00, ne01
	};
	...
	if (!komputeManager()->hasAlgorithm(__func__))
		s_algo = komputeManager()->algorithm<float, PushConstants>(
			...
			{in, out},
			spirv,
			{unsigned(ne00), unsigned(ne01), unsigned(ne02)},
			{},
			{pushConsts}
		);
	else {
...
static void ggml_vk_mul_mat_f16(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_f16_comp_spv,
	...
	if (!komputeManager()->hasAlgorithm(__func__)) {
		...
		s_algo = komputeManager()->algorithm<uint32_t, PushConstants>(
			...
			{inA, inB, out},
			spirv,
			{unsigned(ne01), ny, unsigned(ne12 * ne13)},
			{local_x},
			{pushConsts}
		);
	} else {
...
static void ggml_vk_mul_mat_mat_f32(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_mat_f32_comp_spv,
	...
	if (!komputeManager()->hasAlgorithm(__func__)) {
		s_algo = komputeManager()->algorithm<uint32_t, PushConstants>(
			...
			{inA, inB, out},
			spirv,
			{unsigned(ne01), unsigned(ne11), unsigned(std::max(ne12, ne02))},
			{local_x},
			{pushConsts}
		);
	} else {
...
static void ggml_vk_mul_mat_impl(
	...
	if (!komputeManager()->hasAlgorithm(name)) {
		...
		s_algo = komputeManager()->algorithm<uint32_t, PushConstants>(
			...
			{inA, inB, out},
			spirv,
			{unsigned((ne01 + 7) / 8), unsigned(ne11), unsigned(ne12 * ne13)},
			{local_x},
			{pushConsts}
		);
	} else {
...
static void ggml_vk_mul_mat_q4_0(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_q4_0_comp_spv,
		kp::shader_data::op_mul_mat_q4_0_comp_spv_len
	);

	ggml_vk_mul_mat_impl(
		spirv,
		"q4_0",
		1 /*We access blocks unaligned*/,
...
static void ggml_vk_mul_mat_q4_1(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_q4_1_comp_spv,
...
static void ggml_vk_mul_mat_q8_0(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_q8_0_comp_spv,
...
static void ggml_vk_mul_mat_q6_k(
	...
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_mul_mat_q6_k_comp_spv,
	...
	if (!komputeManager()->hasAlgorithm(__func__)) {
		...
		s_algo = komputeManager()->algorithm<uint32_t, PushConstants>(
			...
			{inA, inB, out},
			spirv,
			{unsigned((ne01 + 1) / 2), unsigned(ne11), unsigned(ne12)},
			{local_x},
			{pushConsts}
		);
	} else {
...
static void ggml_vk_get_rows(
	...
	struct PushConstants {
		...
		safe_divide(inAOff, element_size),
		safe_divide(inBOff, 4),
		safe_divide(outOff, 4),
		ne00,
		nb01,
...
static void ggml_vk_get_rows_f32(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_getrows_f32_comp_spv,
		kp::shader_data::op_getrows_f32_comp_spv_len
	);

	ggml_vk_get_rows(
		spirv, "f32", sizeof(float), 0, std::forward<Args>(args)...
	);
}
...
static void ggml_vk_get_rows_f16(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_getrows_f16_comp_spv,
		kp::shader_data::op_getrows_f16_comp_spv_len
	);

	ggml_vk_get_rows(
		spirv, "f16", sizeof(half), 0, std::forward<Args>(args)...
	);
}
...
static void ggml_vk_get_rows_q4_0(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_getrows_q4_0_comp_spv,
...
static void ggml_vk_get_rows_q4_1(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_getrows_q4_1_comp_spv,
...
static void ggml_vk_get_rows_q6_k(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_getrows_q6_k_comp_spv,
...
static void ggml_vk_rope(
	...
	static const auto spirv_f16 = getSpirvShader(
		kp::shader_data::op_rope_f16_comp_spv,
		kp::shader_data::op_rope_f16_comp_spv_len
	);
	static const auto spirv_f32 = getSpirvShader(
		kp::shader_data::op_rope_f32_comp_spv,
		kp::shader_data::op_rope_f32_comp_spv_len
	);
	...
	struct PushConstants {
		...
		safe_divide(inAOff, type_size),
		safe_divide(inBOff, 4),
		safe_divide(outOff, type_size),
		n_dims,
		mode,
		n_ctx_orig,
		freq_base,
	...
	auto name =
		std::string(__func__) + (src0t == GGML_TYPE_F16 ? "_f16" : "_f32");
	std::shared_ptr<kp::Algorithm> s_algo = nullptr;
	if (!komputeManager()->hasAlgorithm(name)) {
		s_algo = komputeManager()->algorithm<float, PushConstants>(
			name,
...
static void ggml_vk_cpy(
	...
	struct PushConstants {
		...
		safe_divide(inOff, in_element_size),
		safe_divide(outOff, out_element_size),
		ne00,
		ne01,
		ne02,
		nb00,
	...
	std::string name = std::string(__func__) + "_i_" +
					   std::to_string(in_element_size) + "_o_" +
					   std::to_string(out_element_size);
	std::shared_ptr<kp::Algorithm> s_algo = nullptr;
	if (!komputeManager()->hasAlgorithm(name))
		s_algo = komputeManager()->algorithm<float, PushConstants>(
			name,
...
static void ggml_vk_cpy_f32_f16(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_cpy_f32_f16_comp_spv,
...
static void ggml_vk_cpy_f32_f32(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_cpy_f32_f32_comp_spv,
...
static void ggml_vk_cpy_f16_f16(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_cpy_f16_f16_comp_spv,
...
static void ggml_vk_cpy_f16_f32(Args &&...args) {
	const static auto spirv = getSpirvShader(
		kp::shader_data::op_cpy_f16_f32_comp_spv,
...
static void ggml_vk_graph_compute(
	struct ggml_kompute_context *ctx, struct ggml_cgraph *gf
) {
	const int n_seq = 8;
	...
	for (int seq_idx = 0; seq_idx < n_seq; ++seq_idx) {
		...
		const int node_end	 = std::min(
			  (seq_idx == n_seq - 1) ? gf->n_nodes
									 : (seq_idx + 1) * n_nodes_per_seq,
			gf->n_nodes
		);
		...
		for (int i = node_start; i < node_end; ++i) {
			...
			uint32_t off_src1									= 0;
			uint32_t off_dst									= 0;
			const std::shared_ptr<kp::Tensor> &id_src0 =
				src0 ? ggml_vk_get_tensor(src0, &off_src0) : nullTensor;
			const std::shared_ptr<kp::Tensor> &id_src1 =
				src1 ? ggml_vk_get_tensor(src1, &off_src1) : nullTensor;
			const std::shared_ptr<kp::Tensor> &id_dst =
				dst ? ggml_vk_get_tensor(dst, &off_dst) : nullTensor;

			switch (dst->op) {
			case GGML_OP_ADD: {
				if (ggml_nelements(src1) == ne10 && ggml_is_contiguous(src1) &&
					ne00 % 4 == 0 && ne10 % 4 == 0) {
					// src1 is a row
					ggml_vk_addrow(
						seq,
			...
			case GGML_OP_UNARY: {
				...
				case GGML_UNARY_OP_SILU: {
					ggml_vk_silu(
						seq, id_src0, id_dst, off_src0, off_dst, n / 4
					);
				} break;
				case GGML_UNARY_OP_RELU: {
					ggml_vk_relu(
				...
				case GGML_UNARY_OP_GELU: {
					GGML_ASSERT(n % 8 == 0);
					ggml_vk_gelu(
						seq, id_src0, id_dst, off_src0, off_dst, n / 8
					);
				} break;
				default: {
					fprintf(
...
#pragma message("ref:  https://github.com/ggerganov/llama.cpp/pull/7192")
			...
			case GGML_OP_MUL_MAT: {
				...
				if (ggml_is_transposed(src0) || ggml_is_transposed(src1)) {
					fprintf(
						stderr,
						"%s: %s: matmul on tranposed tensor not supported: "
						"%u/%u\n",
						__func__,
						ggml_op_name(dst->op),
						src0t,
				...
				default: {
					fprintf(
						stderr,
						"%s: %s: Unsupported quantization: %u/%u\n",
						__func__,
						ggml_op_name(dst->op),
						src0t,
						src1t
			...
			case GGML_OP_GET_ROWS: {
				if (src0t == GGML_TYPE_F32) {
					...
					fprintf(
						stderr,
						"%s: %s: Unsupported quantization: %u\n",
						__func__,
						ggml_op_name(dst->op),
						src0t
					);
...
#pragma message("      https://github.com/ggerganov/llama.cpp/pull/7225")
				GGML_ASSERT(
					dst->src[2] == nullptr &&
					"phi3 frequency factors not implemented yet"
				);
...
#pragma message("      https://github.com/ggerganov/llama.cpp/pull/7634")
				...
				float freq_base, freq_scale, ext_factor, attn_factor, beta_fast,
					beta_slow;
				memcpy(
					&freq_base, (int32_t *)dst->op_params + 5, sizeof(float)
				);
				memcpy(
					&freq_scale, (int32_t *)dst->op_params + 6, sizeof(float)
				);
				memcpy(
					&ext_factor, (int32_t *)dst->op_params + 7, sizeof(float)
				);
				memcpy(
					&attn_factor, (int32_t *)dst->op_params + 8, sizeof(float)
				);
				memcpy(
					&beta_fast, (int32_t *)dst->op_params + 9, sizeof(float)
				);
				memcpy(
					&beta_slow, (int32_t *)dst->op_params + 10, sizeof(float)
				);
				ggml_vk_rope(
					seq,
					id_src0,
...
template <>
kp::Tensor::TensorDataTypes kp::TensorT<half>::dataType() {
	return TensorDataTypes::eFloat;
}

template <>
kp::Tensor::TensorDataTypes kp::TensorT<uint8_t>::dataType() {
...
struct ggml_backend_kompute_buffer_type_context {
	...
	ggml_backend_kompute_buffer_type_context(
		int device, uint64_t buffer_alignment, uint64_t max_alloc
	) :
		device(device),
...
static void ggml_backend_kompute_device_ref(ggml_backend_buffer_type_t buft) {
	auto *ctx =
		static_cast<ggml_backend_kompute_buffer_type_context *>(buft->context);
...
static const char *ggml_backend_kompute_buffer_get_name(
	ggml_backend_buffer_t buffer
) {
	auto *ctx = static_cast<ggml_backend_kompute_buffer_type_context *>(
		buffer->buft->context
	);
...
static void ggml_backend_kompute_buffer_set_tensor(
	ggml_backend_buffer_t buffer,
	ggml_tensor *tensor,
	const void *data,
	size_t offset,
...
static void ggml_backend_kompute_buffer_get_tensor(
	ggml_backend_buffer_t buffer,
...
static void ggml_backend_kompute_buffer_clear(
	...
	if (memory->stagingBuffer)
		komputeManager()->sequence()->eval<kp::OpBufferSyncDevice>(
			memory->primaryBuffer, memory->stagingBuffer, memory->size
		);
}
...
static ggml_backend_buffer_t ggml_backend_kompute_buffer_type_alloc_buffer(
	...
	return ggml_backend_buffer_init(
		buft, ggml_backend_kompute_buffer_i, ctx, size
	);
}

static size_t ggml_backend_kompute_buffer_type_get_alignment(
	ggml_backend_buffer_type_t buft
) {
	auto *ctx =
		static_cast<ggml_backend_kompute_buffer_type_context *>(buft->context);
...
static size_t ggml_backend_vk_buffer_type_get_max_size(
	ggml_backend_buffer_type_t buft
) {
	auto *ctx =
		static_cast<ggml_backend_kompute_buffer_type_context *>(buft->context);
...
ggml_backend_buffer_type_t ggml_backend_kompute_buffer_type(int device) {
		...
		for (const auto &dev : devices) {
			vec.push_back(
				{/* .iface   = */ ggml_backend_kompute_buffer_type_interface,
				 /* .context = */ new ggml_backend_kompute_buffer_type_context(
					 dev.index, dev.bufferAlignment, dev.maxAlloc
				 )}
			);
		}
		return vec;
	...
	auto it = std::find_if(
		bufts.begin(),
		bufts.end(),
		[device](const ggml_backend_buffer_type &t) {
			return device ==
				   static_cast<ggml_backend_kompute_buffer_type_context *>(
					   t.context
				   )
					   ->device;
		}
...
static ggml_backend_buffer_type_t ggml_backend_kompute_get_default_buffer_type(
...
static bool ggml_backend_kompute_supports_op(
...
static bool ggml_backend_kompute_supports_buft(
...
static struct ggml_backend_i kompute_backend_i = {
	...
	ggml_backend_kompute_get_default_buffer_type,
...
bool ggml_backend_is_kompute(ggml_backend_t backend) {
	return backend != NULL &&
		   ggml_guid_matches(backend->guid, ggml_backend_kompute_guid());
}

static ggml_backend_t ggml_backend_reg_kompute_init(
