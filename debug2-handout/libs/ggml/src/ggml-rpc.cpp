...
static_assert(
	sizeof(rpc_tensor) % 8 == 0, "rpc_tensor size must be multiple of 8"
);
...
static bool set_no_delay(sockfd_t sockfd) {
	...
	int ret = setsockopt(
		sockfd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag, sizeof(int)
	);
	return ret == 0;
}
...
static bool set_reuse_addr(sockfd_t sockfd) {
	int flag = 1;
	int ret	 = setsockopt(
		 sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&flag, sizeof(int)
	 );
	return ret == 0;
}
...
static std::shared_ptr<socket_t> create_server_socket(
...
static bool send_data(sockfd_t sockfd, const void *data, size_t size) {
	...
	while (bytes_sent < size) {
		ssize_t n =
			send(sockfd, (const char *)data + bytes_sent, size - bytes_sent, 0);
		if (n < 0) {
			return false;
		}
		bytes_sent += n;
...
static bool recv_data(sockfd_t sockfd, void *data, size_t size) {
	...
	while (bytes_recv < size) {
		ssize_t n =
			recv(sockfd, (char *)data + bytes_recv, size - bytes_recv, 0);
		if (n <= 0) {
			return false;
		}
		bytes_recv += n;
...
static bool parse_endpoint(
	const std::string &endpoint, std::string &host, int &port
) {
...
GGML_CALL static const char *ggml_backend_rpc_buffer_get_name(
...
GGML_CALL static void ggml_backend_rpc_buffer_free_buffer(
	ggml_backend_buffer_t buffer
) {
	ggml_backend_rpc_buffer_context *ctx =
		(ggml_backend_rpc_buffer_context *)buffer->context;
...
GGML_CALL static void *ggml_backend_rpc_buffer_get_base(
	ggml_backend_buffer_t buffer
) {
	ggml_backend_rpc_buffer_context *ctx =
		(ggml_backend_rpc_buffer_context *)buffer->context;
	...
	std::vector<uint8_t> output;
	bool status =
		send_rpc_cmd(ctx->sock, RPC_CMD_BUFFER_GET_BASE, input, output);
	GGML_ASSERT(status);
...
GGML_CALL static void ggml_backend_rpc_buffer_init_tensor(
...
GGML_CALL static void ggml_backend_rpc_buffer_set_tensor(
	ggml_backend_buffer_t buffer,
	ggml_tensor *tensor,
	const void *data,
	size_t offset,
...
GGML_CALL static void ggml_backend_rpc_buffer_get_tensor(
	ggml_backend_buffer_t buffer,
	...
	memcpy(input.data() + sizeof(rpc_tensor), &offset, sizeof(offset));
	memcpy(
		input.data() + sizeof(rpc_tensor) + sizeof(offset), &size, sizeof(size)
	);
	std::vector<uint8_t> output;
...
GGML_CALL static bool ggml_backend_rpc_buffer_cpy_tensor(
...
GGML_CALL static void ggml_backend_rpc_buffer_clear(
...
GGML_CALL static const char *ggml_backend_rpc_buffer_type_name(
	...
	if (remote_ptr != 0) {
		ggml_backend_buffer_t buffer = ggml_backend_buffer_init(
			...
			new ggml_backend_rpc_buffer_context{
				...
				{},
				remote_ptr,
				"RPC[" + std::string(buft_ctx->endpoint) + "]"
			},
			remote_size
		);
		return buffer;
...
GGML_CALL static size_t ggml_backend_rpc_buffer_type_get_alignment(
	ggml_backend_buffer_type_t buft
) {
	ggml_backend_rpc_buffer_type_context *buft_ctx =
		(ggml_backend_rpc_buffer_type_context *)buft->context;
...
GGML_CALL static size_t ggml_backend_rpc_get_max_size(
	ggml_backend_buffer_type_t buft
) {
	ggml_backend_rpc_buffer_type_context *buft_ctx =
		(ggml_backend_rpc_buffer_type_context *)buft->context;
...
GGML_CALL static size_t ggml_backend_rpc_buffer_type_get_alloc_size(
...
static void serialize_graph(
	...
	int output_size	   = sizeof(uint32_t) + n_nodes * sizeof(uint64_t) +
					  sizeof(uint32_t) + n_tensors * sizeof(rpc_tensor);
	output.resize(output_size, 0);
	memcpy(output.data(), &n_nodes, sizeof(n_nodes));
	for (uint32_t i = 0; i < n_nodes; i++) {
		memcpy(
			output.data() + sizeof(n_nodes) + i * sizeof(uint64_t),
			&cgraph->nodes[i],
			sizeof(uint64_t)
		);
	}
	uint32_t *out_ntensors = (uint32_t *)(output.data() + sizeof(n_nodes) +
										  n_nodes * sizeof(uint64_t));
	*out_ntensors		   = n_tensors;
	rpc_tensor *out_tensors =
		(rpc_tensor *)(output.data() + sizeof(n_nodes) +
					   n_nodes * sizeof(uint64_t) + sizeof(uint32_t));
	memcpy(out_tensors, tensors.data(), n_tensors * sizeof(rpc_tensor));
}

GGML_CALL static enum ggml_status ggml_backend_rpc_graph_compute(
...
GGML_CALL static bool ggml_backend_rpc_supports_op(
...
GGML_CALL static bool ggml_backend_rpc_supports_buft(
...
ggml_backend_rpc_buffer_type(const char *endpoint) {
	...
	ggml_backend_rpc_buffer_type_context *buft_ctx =
		new ggml_backend_rpc_buffer_type_context{
			/* .endpoint  = */ endpoint,
...
static void get_device_memory(
	const std::shared_ptr<socket_t> &sock, size_t *free, size_t *total
) {
	// input serialization format: | 0 bytes |
	std::vector<uint8_t> input;
	std::vector<uint8_t> output;
...
GGML_API GGML_CALL void ggml_backend_rpc_get_device_memory(
	const char *endpoint, size_t *free, size_t *total
) {
	auto sock = get_socket(endpoint);
	if (sock == nullptr) {
		*free  = 0;
...
private:
	ggml_tensor *deserialize_tensor(
		struct ggml_context *ctx, const rpc_tensor *tensor
	);
...
bool rpc_server::alloc_buffer(
	...
	if (buffer != nullptr) {
		...
		GGML_PRINT_DEBUG(
			"[%s] size: %" PRIu64 " -> remote_ptr: %" PRIx64
			", remote_size: %" PRIu64 "\n",
			__func__,
			size,
			remote_ptr,
...
bool rpc_server::buffer_get_base(
	const std::vector<uint8_t> &input, std::vector<uint8_t> &output
) {
	// input serialization format: | remote_ptr (8 bytes) |
	if (input.size() != sizeof(uint64_t)) {
	...
	ggml_backend_buffer_t buffer =
		reinterpret_cast<ggml_backend_buffer_t>(remote_ptr);
...
ggml_tensor *rpc_server::deserialize_tensor(
	struct ggml_context *ctx, const rpc_tensor *tensor
) {
...
bool rpc_server::get_tensor(
	...
	uint64_t size;
	memcpy(
		&size, input.data() + sizeof(rpc_tensor) + sizeof(offset), sizeof(size)
	);
	...
	GGML_PRINT_DEBUG(
		"[%s] buffer: %p, data: %p, offset: %" PRIu64 ", size: %" PRIu64 "\n",
		__func__,
		(void *)tensor->buffer,
		tensor->data,
		offset,
...
bool rpc_server::copy_tensor(
	...
	const rpc_tensor *rpc_dst =
		(const rpc_tensor *)(input.data() + sizeof(rpc_src));
...
bool rpc_server::graph_compute(
	...
	if (input.size() <
		sizeof(uint32_t) + n_nodes * sizeof(uint64_t) + sizeof(uint32_t)) {
		return false;
	}
	...
	memcpy(
		&n_tensors,
		input.data() + sizeof(n_nodes) + n_nodes * sizeof(uint64_t),
		sizeof(n_tensors)
	);
	if (input.size() < sizeof(uint32_t) + n_nodes * sizeof(uint64_t) +
						   sizeof(uint32_t) + n_tensors * sizeof(rpc_tensor)) {
		return false;
	}
	const rpc_tensor *tensors =
		(const rpc_tensor *)(input.data() + sizeof(n_nodes) +
							 n_nodes * sizeof(uint64_t) + sizeof(n_tensors));
	GGML_PRINT_DEBUG(
		"[%s] n_nodes: %u, n_tensors: %u\n", __func__, n_nodes, n_tensors
	);
...
static void rpc_serve_client(
	...
	while (true) {
		...
		try {
			...
			fprintf(
				stderr,
				"Failed to allocate input buffer of size %" PRIu64 "\n",
				input_size
			);
			break;
		}
		...
		case RPC_CMD_GET_DEVICE_MEMORY: {
			...
			memcpy(
				output.data() + sizeof(uint64_t), &total_mem, sizeof(total_mem)
			);
			break;
		}
		default: {
...
void start_rpc_server(
	...
	while (true) {
		...
		printf(
			"Accepted client connection, free_mem=%zu, total_mem=%zu\n",
			free_mem,
			total_mem
		);
		fflush(stdout);
