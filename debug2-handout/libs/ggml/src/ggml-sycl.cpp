...
void ggml_sycl_get_device_description(
...
typedef void (*ggml_sycl_op_flatten_t)(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
template <
	float (*bin_op)(const float, const float),
	typename src0_t,
	typename src1_t,
	typename dst_t>
static void k_bin_bcast(
	const src0_t *src0,
	const src1_t *src1,
	dst_t *dst,
	int ne0,
	...
	for (int i0 = i0s; i0 < ne0;
		...
		dst_row[i0]	  = (dst_t
		  )bin_op(src0 ? (float)src0_row[i0] : 0.0f, (float)src1_row[i10]);
	}
}
...
static void gelu_f32(
	const float *x, float *dst, const int k, const sycl::nd_item<3> &item_ct1
) {
...
static void gelu_quick_f32(
	const float *x, float *dst, int k, const sycl::nd_item<3> &item_ct1
) {
...
static void upscale_f32(
	...
	dst[index] = *(float *)((char *)x + i03 * nb03 + i02 * nb02 + i01 * nb01 +
							i00 * nb00);
}
...
static void mul_mat_p021_f16_f32(
	...
	for (int col_x0 = 0; col_x0 < ncols_x;
		...
		const int ix =
			row_x * nchannels_x * ncols_x + channel_x * ncols_x + col_x;
		const float xi =
...
static void mul_mat_vec_nc_f16_f32( // nc == non-contiguous
	...
	for (int col_x0 = 0; col_x0 < ncols_x;
		...
		const int ix =
			channel_x * channel_stride_x + row_x * row_stride_x + col_x;
		const int iy = channel * nrows_y + row_y;
...
static void cpy_f32_f16(
	...
	const int i00 =
		i - i03 * ne00 * ne01 * ne02 - i02 * ne01 * ne00 - i01 * ne00;
	...
	const int i10 =
		i - i13 * ne10 * ne11 * ne12 - i12 * ne10 * ne11 - i11 * ne10;
...
static void cpy_f32_q(
	...
	const int dst_offset =
		(i10 / qk) * nb10 + i11 * nb11 + i12 * nb12 + i13 * nb13;

	cpy_blck(cx + x_offset, cdst + dst_offset);
}

static void k_sum_rows_f32(
	const float *x,
	float *dst,
	const int ncols,
	const sycl::nd_item<3> &item_ct1
...
__dpct_inline__ static void k_argsort_f32_i32(
	const float *x,
	int *dst,
	const int ncols,
	int ncols_pad,
...
static void get_rows_sycl_float(
...

	{
		dpct::has_capability_or_fail(
			stream->get_device(), {sycl::aspect::fp16}
		);
...
static void acc_f32_sycl(
	...
	int num_blocks =
		(n_elements + SYCL_ACC_BLOCK_SIZE - 1) / SYCL_ACC_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
...
static void gelu_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_GELU_BLOCK_SIZE - 1) / SYCL_GELU_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
...
static void silu_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_SILU_BLOCK_SIZE - 1) / SYCL_SILU_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
...
static void gelu_quick_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_GELU_BLOCK_SIZE - 1) / SYCL_GELU_BLOCK_SIZE;
...
static void tanh_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_TANH_BLOCK_SIZE - 1) / SYCL_TANH_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
...
static void relu_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_RELU_BLOCK_SIZE - 1) / SYCL_RELU_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
...
static void hardsigmoid_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_HARDSIGMOID_BLOCK_SIZE - 1) / SYCL_HARDSIGMOID_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
		...
		[=](sycl::nd_item<3> item_ct1) { hardsigmoid_f32(x, dst, k, item_ct1); }
	);
}

static void hardswish_f32_sycl(
	const float *x, float *dst, const int k, queue_ptr stream
) {
	const int num_blocks =
		(k + SYCL_HARDSWISH_BLOCK_SIZE - 1) / SYCL_HARDSWISH_BLOCK_SIZE;
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks) *
		...
		[=](sycl::nd_item<3> item_ct1) { hardswish_f32(x, dst, k, item_ct1); }
	);
}

static void leaky_relu_f32_sycl(
	const float *x,
	float *dst,
	const int k,
	const float negative_slope,
...
static void upscale_f32_sycl(
	...
	int num_blocks =
		(dst_size + SYCL_UPSCALE_BLOCK_SIZE - 1) / SYCL_UPSCALE_BLOCK_SIZE;
...
static void pad_f32_sycl(
	...
	stream->parallel_for(
		sycl::nd_range<3>(
			gridDim * sycl::range<3>(1, 1, SYCL_PAD_BLOCK_SIZE),
...
static void quantize_row_q8_1_sycl(
	const float *x,
	void *vy,
	const int kx,
	const int ky,
	...
	const sycl::range<3> block_size(
		1, 1, SYCL_QUANTIZE_BLOCK_SIZE / QUANT_BLOCK_TILE
	);
	{
		dpct::has_capability_or_fail(
			stream->get_device(), {sycl::aspect::fp16}
...
static void ggml_mul_mat_p021_f16_f32_sycl(
	const void *vx,
	const float *y,
	float *dst,
	const int ncols_x,
...
static void ggml_cpy_f32_q8_0_sycl(
	...
	stream->parallel_for(
		sycl::nd_range<3>(
			sycl::range<3>(1, 1, num_blocks), sycl::range<3>(1, 1, 1)
		),
		[=](sycl::nd_item<3> item_ct1) {
...
static void ggml_cpy_f32_q4_0_sycl(
	const char *cx,
	char *cdst,
	const int ne,
	const int ne00,
...
static void ggml_cpy_f32_q4_1_sycl(
	const char *cx,
	char *cdst,
	const int ne,
	const int ne00,
...
static void ggml_cpy_f16_f16_sycl(
	const char *cx,
	char *cdst,
	const int ne,
	const int ne00,
...
static void ggml_cpy_i16_i16_sycl(
	const char *cx,
	char *cdst,
	const int ne,
	const int ne00,
...
static void ggml_cpy_i32_i32_sycl(
	const char *cx,
	char *cdst,
	const int ne,
	const int ne00,
...
static void scale_f32_sycl(
	const float *x, float *dst, const float scale, const int k, queue_ptr stream
) {
	const int num_blocks =
...
static void argsort_f32_i32_sycl(
	const float *x,
	int *dst,
	const int ncols,
	const int nrows,
...
static void diag_mask_inf_f32_sycl(
	const float *x,
	float *dst,
	const int ncols_x,
	const int nrows_x,
	...
	stream->parallel_for(
		...
		[=](sycl::nd_item<3> item_ct1) {
			diag_mask_inf_f32(
				x, dst, ncols_x, rows_per_channel, n_past, item_ct1
			);
		}
	);
}
...
void print_device_detail(
	int id, sycl::device &device, std::string device_type
) {
	...
	device_type =
		std::regex_replace(device_type, std::regex("ext_oneapi_"), "");
	...
	fprintf(
		stderr,
		"|%2d|%19s|%39s|%7s|%7d|%8d|%5d|%6luM|%21s|\n",
		id,
		device_type.c_str(),
		name.c_str(),
		version.c_str(),
...
static ggml_sycl_device_info ggml_sycl_init() {
	...
	if (info.device_count == 0) {
		fprintf(
			stderr,
			"%s: failed to initialize " GGML_SYCL_NAME ": %s\n",
			__func__
		);
		return info;
	}
	...
	fprintf(
		stderr,
		"%s: found %d " GGML_SYCL_NAME " devices:\n",
		__func__,
		info.device_count
	);
...
struct ggml_sycl_pool_leg : public ggml_sycl_pool {
	...
	void *alloc(size_t size, size_t *actual_size) override {
		...
		if (!ptr) {
			fprintf(
				stderr,
				"%s: can't malloc %lu Bytes memory on device",
				__func__,
				look_ahead_size
			);
			return nullptr;
...
std::unique_ptr<ggml_sycl_pool> ggml_backend_sycl_context::new_pool_for_device(
	queue_ptr qptr, int device
) {
	// TBD: NO VMM support
...
static dpct::err0 ggml_sycl_cpy_tensor_2d(
	void *dst,
	...
	} else if (src->backend == GGML_BACKEND_TYPE_GPU ||
...
static void ggml_sycl_op_get_rows(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	case GGML_TYPE_F32:
		get_rows_sycl_float(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	case GGML_TYPE_Q4_0:
		get_rows_sycl<QK4_0, QR4_0, dequantize_q4_0>(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	case GGML_TYPE_Q4_1:
		get_rows_sycl<QK4_1, QR4_1, dequantize_q4_1>(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	case GGML_TYPE_Q5_0:
		get_rows_sycl<QK5_0, QR5_0, dequantize_q5_0>(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	case GGML_TYPE_Q5_1:
		get_rows_sycl<QK5_1, QR5_1, dequantize_q5_1>(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	case GGML_TYPE_Q8_0:
		get_rows_sycl<QK8_0, QR8_0, dequantize_q8_0>(
			ctx, src0, src1, dst, src0_d, src1_i32, dst_d, stream
		);
		break;
	default:
...
template <class op>
inline void ggml_sycl_op_bin_bcast(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
		...
		op()(
			...
			src1,
			dst,
			(const int16_t *)src0_dd,
			(const int16_t *)src1_dd,
			(int16_t *)dst_dd,
			main_stream
		);
	} else {
...
static void ggml_sycl_op_repeat(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_repeat>>(
		ctx, dst, src0, dst, nullptr, src0_d, dst_d, main_stream
	);
...
inline void ggml_sycl_op_add(
	...
	ggml_sycl_op_bin_bcast<bin_bcast_sycl<op_add>>(
		ctx, src0, src1, dst, src0_dd, src1_dd, dst_dd, main_stream
	);
}
...
inline void ggml_sycl_op_gelu_quick(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
static void ggml_sycl_op_hardsigmoid(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
static void ggml_sycl_op_hardswish(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
inline void ggml_sycl_op_leaky_relu(
	...
	leaky_relu_f32_sycl(
		src0_dd, dst_dd, ggml_nelements(src0), negative_slope, main_stream
	);
...
inline void ggml_sycl_op_upscale(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
static int64_t get_row_rounding(
	ggml_type type, const std::array<float, GGML_SYCL_MAX_DEVICES> &tensor_split
) {
	int64_t min_compute_capability = INT_MAX;
	int64_t max_compute_capability = INT_MIN;
	for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
		if (tensor_split[i] < (i + 1 < ggml_sycl_info().device_count
								   ? tensor_split[i + 1]
								   : 1.0f)) {
...
inline void ggml_sycl_op_mul_mat_sycl(
	...
	if ((src0->type == GGML_TYPE_F16 || ggml_is_quantized(src0->type)) &&
		...
		if (src0->type != GGML_TYPE_F16) {
			const to_fp16_sycl_t to_fp16_sycl =
				ggml_get_to_fp16_sycl(src0->type);
		...
		if (src1->type != GGML_TYPE_F16) {
			const to_fp16_sycl_t to_fp16_sycl =
				ggml_get_to_fp16_sycl(src1->type);
			GGML_ASSERT(to_fp16_sycl != nullptr);
		...
		const to_fp32_sycl_t to_fp32_sycl =
			ggml_get_to_fp32_sycl(GGML_TYPE_F16);
		...
		if (src0->type != GGML_TYPE_F32) {
			const to_fp32_sycl_t to_fp32_sycl =
				ggml_get_to_fp32_sycl(src0->type);
			GGML_ASSERT(to_fp32_sycl != nullptr);
			src0_ddq_as_f32.alloc(row_diff * ne00);
			to_fp32_sycl(
				src0_dd_i, src0_ddq_as_f32.get(), row_diff * ne00, stream
			);
		}
		if (src1->type != GGML_TYPE_F32) {
			const to_fp32_sycl_t to_fp32_sycl =
				ggml_get_to_fp32_sycl(src1->type);
			GGML_ASSERT(to_fp32_sycl != nullptr);
			src1_ddq_as_f32.alloc(src1_ncols * ne10);
			to_fp32_sycl(
				src1_ddf_i, src1_ddq_as_f32.get(), src1_ncols * ne10, stream
			);
		}
		const float *src0_ddf_i	 = src0->type == GGML_TYPE_F32
									   ? (const float *)src0_dd_i
									   : src0_ddq_as_f32.get();
		const float *src1_ddf1_i = src1->type == GGML_TYPE_F32
									   ? (const float *)src1_ddf_i
									   : src1_ddq_as_f32.get();
...
static void ggml_sycl_op_pool2d(
	...
	const int num_blocks = (parallel_elements + SYCL_POOL2D_BLOCK_SIZE - 1) /
						   SYCL_POOL2D_BLOCK_SIZE;
...
inline void ggml_sycl_op_sum_rows(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
inline void ggml_sycl_op_argsort(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	argsort_f32_i32_sycl(
		src0_dd, (int *)dst_dd, ncols, nrows, order, main_stream
	);
...
inline void ggml_sycl_op_diag_mask_inf(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	diag_mask_inf_f32_sycl(
		src0_dd, dst_dd, ne00, nrows0, ne01, n_past, main_stream
	);
...
inline void ggml_sycl_op_scale(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
...
inline void ggml_sycl_op_clamp(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst,
	...
	clamp_f32_sycl(
		src0_dd, dst_dd, min, max, ggml_nelements(src0), main_stream
	);
	/*
...
static void ggml_sycl_set_peer_access(const int n_tokens, int main_device) {
	...
	for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
		...
		for (int id_other = 0; id_other < ggml_sycl_info().device_count;
			 ++id_other) {
			if (i == id_other) {
				continue;
			}
...
static void ggml_sycl_op_mul_mat(
	...
	for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
		...
		if (src0_is_contiguous) {
			...
			dev[i].src0_dd =
				dev[i].src0_dd_alloc.alloc(ctx.pool(i), ggml_nbytes(src0));
		}
		...
		if (src1_on_device && src1_is_contiguous) {
			...
			dev[i].src1_ddf =
				dev[i].src1_ddf_alloc.alloc(ctx.pool(i), ggml_nelements(src1));
		}

		if (convert_src1_to_q8_1) {
			dev[i].src1_ddq = dev[i].src1_ddq_alloc.alloc(
				ctx.pool(i), nrows1 * src1_padded_col_size * q8_1_ts / q8_1_bs
			);
		...
		if (dst_on_device) {
			...
			const size_t size_dst_ddf =
				split ? (dev[i].row_high - dev[i].row_low) * ne1
					  : ggml_nelements(dst);
			dev[i].dst_dd =
				dev[i].dst_dd_alloc.alloc(ctx.pool(i), size_dst_ddf);
		}
	}
	...
	for (int64_t src1_col_0 = 0; src1_col_0 < ne11;
		...
		const int64_t is =
			split ? (src1_col_0 / src1_col_stride) % GGML_SYCL_MAX_STREAMS : 0;
		const int64_t src1_ncols = src1_col_0 + src1_col_stride > ne11
									   ? ne11 - src1_col_0
									   : src1_col_stride;

		for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
			if ((!split && i != ctx.device) ||
				dev[i].row_low == dev[i].row_high) {
				continue;
			}
			...
			for (int64_t i0 = 0; i0 < ne13 * ne12; ++i0) {
				...
				const size_t src1_ddq_i_offset = (i0 * ne11 + src1_col_0) *
												 src1_padded_col_size *
												 q8_1_ts / q8_1_bs;

				// for split tensors the data begins at i0 == i0_offset_low
				char *src0_dd_i = dev[i].src0_dd + (i0 / i02_divisor) *
													   (ne01 * ne00 * src0_ts) /
													   src0_bs;
				float *src1_ddf_i =
					dev[i].src1_ddf + (i0 * ne11 + src1_col_0) * ne10;
				char *src1_ddq_i = dev[i].src1_ddq + src1_ddq_i_offset;
				float *dst_dd_i =
					dev[i].dst_dd +
					(i0 * ne1 + src1_col_0) * (dst_on_device ? ne0 : row_diff);

				// the main device memory buffer can be on VRAM scratch, with space for all partial results
				// in that case an offset on dst_ddf_i is needed
				if (i == ctx.device) {
					dst_dd_i +=
						dev[i].row_low; // offset is 0 if no tensor split
				}
					...
					if (i != ctx.device) {
						if (convert_src1_to_q8_1) {
							char *src1_ddq_i_source =
								dev[ctx.device].src1_ddq + src1_ddq_i_offset;
							SYCL_CHECK(CHECK_TRY_ERROR(
								stream
									->memcpy(
										src1_ddq_i,
										src1_ddq_i_source,
										src1_ncols * src1_padded_col_size *
											q8_1_ts / q8_1_bs
									)
									.wait()
							));
							...
							float *src1_ddf_i_source =
								(float *)src1_extra->data_device[ctx.device];
							src1_ddf_i_source +=
								(i0 * ne11 + src1_col_0) * ne10;

							SYCL_CHECK(CHECK_TRY_ERROR(dev2dev_memcpy(
								*stream,
								*main_stream,
								src1_ddf_i,
								src1_ddf_i_source,
				...
				if (src1_col_0 == 0 && !src0_is_contiguous &&
					i02 % i02_divisor == 0) {
					SYCL_CHECK(ggml_sycl_cpy_tensor_2d(
						src0_dd_i,
						src0,
	...
	if (split && ggml_sycl_info().device_count > 1) {
		...
		is_max =
			is_max <= GGML_SYCL_MAX_STREAMS ? is_max : GGML_SYCL_MAX_STREAMS;
		...
		for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
			...
			for (int64_t is = 0; is < is_max; ++is) {
				SYCL_CHECK(
					CHECK_TRY_ERROR(ctx.stream()->ext_oneapi_submit_barrier(
						{*src0_extra->events[i][is]}
					))
				);
			}
...
static void ggml_sycl_get_rows(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_gelu_quick(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_hardsigmoid(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_hardswish(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_leaky_relu(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_group_norm(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_rms_norm(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_mul_mat_vec_p021(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
	...
	ggml_mul_mat_p021_f16_f32_sycl(
		src0_ddq, src1_ddf, dst_ddf, ne00, ne01, ne02, ne12, main_stream
	);
} catch (sycl::exception const &exc) {
...
static void ggml_sycl_mul_mat_vec_nc(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
	...
	ggml_mul_mat_vec_nc_f16_f32_sycl(
		src0_ddq,
		src1_ddf,
		dst_ddf,
		ne00,
...
static void k_compute_batched_ptrs(
	const sycl::half *src0_as_f16,
	...
	ptrs_src[1 * ne23 + i12 + i13 * ne12] =
		(const char *)src1_as_f16 + i12 * nb12 + i13 * nb13;
	ptrs_dst[0 * ne23 + i12 + i13 * ne12] =
		(char *)dst + i12 * nbd2 + i13 * nbd3;
}
...
static void ggml_sycl_mul_mat_batched_sycl(
	...
	if (r2 == 1 && r3 == 1 && ggml_is_contiguous_2(src0) &&
        ...
        */
		{
			dpct::has_capability_or_fail(
				main_stream->get_device(), {sycl::aspect::fp16}
			);
...
static void ggml_sycl_mul_mat(
	...
	if (split) {
		...
		for (int id = 0; id < ggml_sycl_info().device_count; ++id) {
			...
			if (tensor_split[id] >= (id + 1 < ggml_sycl_info().device_count
										 ? tensor_split[id + 1]
										 : 1.0f)) {
				continue;
			}
	...
	bool use_dequantize_mul_mat_vec =
		ggml_sycl_supports_dmmv(src0->type) && src1->type == GGML_TYPE_F32 &&
		dst->type == GGML_TYPE_F32 && src0->ne[0] % GGML_SYCL_DMMV_X == 0 &&
		src1->ne[1] == 1;
	...
	if (!split && src0->type == GGML_TYPE_F16 && ggml_is_permuted(src0) &&
		ggml_is_permuted(src1) && src1->ne[1] == 1) {
		// KQ single-batch
		ggml_sycl_mul_mat_vec_p021(ctx, src0, src1, dst);
	} else if (!split && src0->type == GGML_TYPE_F16 &&
			   !ggml_is_contiguous(src0) && !ggml_is_transposed(src1) &&
			   src1->ne[1] == 1) {
		// KQV single-batch
		...
		ggml_sycl_op_mul_mat(
			ctx, src0, src1, dst, ggml_sycl_op_dequantize_mul_mat_vec, false
		);
	} else if (use_mul_mat_vec_q) {
		ggml_sycl_op_mul_mat(
			ctx, src0, src1, dst, ggml_sycl_op_mul_mat_vec_q, true
		);
	} else if (use_mul_mat_q) {
		ggml_sycl_op_mul_mat(
			ctx, src0, src1, dst, ggml_sycl_op_mul_mat_q, true
		);
	} else {
		ggml_sycl_op_mul_mat(
			ctx, src0, src1, dst, ggml_sycl_op_mul_mat_sycl, false
		);
	}
}
...
__dpct_inline__ static void k_copy_src1_to_contiguous(
	...
	const int32_t row_id_i =
		*(const int32_t *)(ids + iid1 * ids_nb1 + id * ids_nb0);
...
static void ggml_sycl_mul_mat_id(
	...
	if (ne12 == 1) {
		...
		ggml_sycl_pool_alloc<char> src1_contiguous(
			ctx.pool(), sizeof(float) * ggml_nelements(src1)
		);
		ggml_sycl_pool_alloc<char> dst_contiguous(
		...
		for (int64_t i02 = 0; i02 < n_as; i02++) {
			...
			ggml_sycl_pool_alloc<mmid_row_mapping> dev_row_mapping(
				ctx.pool(), num_src1_rows
			);
			SYCL_CHECK(CHECK_TRY_ERROR(
...

			{
				sycl::range<3> block_dims(
					1, 1, std::min((unsigned int)ne10, 768u)
				);
			...
			{
				sycl::range<3> block_dims(
					1, 1, std::min((unsigned int)ne0, 768u)
				);
...
static void ggml_sycl_diag_mask_inf(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_soft_max(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_rope(
	...
	); // TODO: this restriction is temporary until non-cont support is implemented
	ggml_sycl_op_flatten(ctx, src0, src1, dst, ggml_sycl_op_rope);
}

static void ggml_sycl_pool2d(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static void ggml_sycl_sum_rows(
	ggml_backend_sycl_context &ctx,
	const ggml_tensor *src0,
	const ggml_tensor *src1,
	ggml_tensor *dst
...
static size_t ggml_nbytes_split(
	const struct ggml_tensor *tensor, int nrows_split
) {
	static_assert(
		GGML_MAX_DIMS == 4, "GGML_MAX_DIMS is not 4 - update this function"
	);
...
void ggml_sycl_set_main_device(const int main_device) try {
	...
	if (g_ggml_sycl_debug) {
		...
		fprintf(
			stderr,
			"Using device %d (%s) as main device\n",
			main_device,
			prop.get_name()
		);
	}
...
bool ggml_sycl_compute_forward(
	ggml_backend_sycl_context &ctx, struct ggml_tensor *tensor
) {
	if (!g_sycl_loaded)
		return false;
	...
	if (tensor->src[0] != nullptr &&
		ggml_backend_buffer_is_sycl_split(tensor->src[0]->buffer)) {
...
GGML_API GGML_CALL void ggml_sycl_get_device_description(
	int device, char *description, size_t description_size
) try {
...
GGML_CALL void ggml_backend_sycl_get_device_memory(
    ...
    */
	SYCL_CHECK(CHECK_TRY_ERROR(dpct::dev_mgr::instance()
								   .get_device(device)
								   .get_memory_info(*free, *total)));
} catch (sycl::exception const &exc) {
...
struct ggml_backend_sycl_buffer_context {
	...
	ggml_backend_sycl_buffer_context(
		int device, void *dev_ptr, queue_ptr stream
	) :
		device(device),
		dev_ptr(dev_ptr),
		stream(stream) {
...
GGML_CALL static void ggml_backend_sycl_buffer_init_tensor(
	...
	if (ggml_is_quantized(tensor->type)) {
		...
		if (padded_size > original_size && tensor->view_src == nullptr) {
			SYCL_CHECK(CHECK_TRY_ERROR(ctx->stream
										   ->memset(
											   (char *)tensor->data +
												   original_size,
											   0,
...
static void ggml_backend_sycl_buffer_set_tensor(
	ggml_backend_buffer_t buffer,
	ggml_tensor *tensor,
	const void *data,
	size_t offset,
	...
	ggml_sycl_set_device(ctx->device);
	auto stream =
		&(dpct::dev_mgr::instance().get_device(ctx->device).default_queue());
	SYCL_CHECK(CHECK_TRY_ERROR(dpct::dev_mgr::instance()
								   .get_device(ctx->device)
								   .queues_wait_and_throw()));
	char *host_buf = (char *)malloc(size);
	memcpy(host_buf, data, size);
	SYCL_CHECK(CHECK_TRY_ERROR(
		(*stream).memcpy((char *)tensor->data + offset, host_buf, size).wait()
	));
	free(host_buf);
} catch (sycl::exception const &exc) {
...
static void ggml_backend_sycl_buffer_get_tensor(
	ggml_backend_buffer_t buffer,
...
GGML_CALL static bool ggml_backend_sycl_buffer_cpy_tensor(
	ggml_backend_buffer_t buffer, const ggml_tensor *src, ggml_tensor *dst
) try {
	if (ggml_backend_buffer_is_sycl(src->buffer)) {
		ggml_backend_sycl_buffer_context *src_ctx =
			(ggml_backend_sycl_buffer_context *)src->buffer->context;
		ggml_backend_sycl_buffer_context *dst_ctx =
			(ggml_backend_sycl_buffer_context *)dst->buffer->context;
...
static void ggml_backend_sycl_buffer_clear(
...
ggml_backend_sycl_buffer_type_alloc_buffer(
	...
	ggml_backend_sycl_buffer_type_context *buft_ctx =
		(ggml_backend_sycl_buffer_type_context *)buft->context;
	...
	ggml_backend_sycl_buffer_context *ctx =
		new ggml_backend_sycl_buffer_context(
			buft_ctx->device, dev_ptr, buft_ctx->stream
		);
	return ggml_backend_buffer_init(
		buft, ggml_backend_sycl_buffer_interface, ctx, size
	);
} catch (sycl::exception const &exc) {
...
GGML_CALL static size_t ggml_backend_sycl_buffer_type_get_alignment(
	ggml_backend_buffer_type_t buft
) {
	return 128;
	UNUSED(buft);
...
static size_t ggml_backend_sycl_buffer_type_get_max_size(
	ggml_backend_buffer_type_t buft
) {
...
GGML_CALL static size_t ggml_backend_sycl_buffer_type_get_alloc_size(
...
ggml_backend_buffer_type_t ggml_backend_sycl_buffer_type(int device) {
	...
	if (device >= ggml_sycl_info().device_count or device < 0) {
		printf(
			"ggml_backend_sycl_buffer_type error: device_index:%d is out of "
			"range [0, %d], miss to call "
	...
	if (!ggml_backend_sycl_buffer_type_initialized) {
		for (int i = 0; i < ggml_sycl_info().device_count; i++) {
			...
			ggml_backend_sycl_buffer_types[i] = {
				...
				/* .context  = */
				new ggml_backend_sycl_buffer_type_context{
					i, GGML_SYCL_NAME + std::to_string(i), stream
				},
			};
		}
	...
	if (!ggml_backend_sycl_buffer_type_initialized) {
		for (int i = 0; i < ggml_sycl_info().device_count; i++) {
			ggml_backend_sycl_buffer_types[i] = {
				...
				/* .context  = */
				new ggml_backend_sycl_buffer_type_context{
					i, GGML_SYCL_NAME + std::to_string(i), ctx->stream(i, 0)
				},
			};
		}
		ggml_backend_sycl_buffer_type_initialized = true;
			...
			for (int i = 0; i < ggml_sycl_info().device_count; ++i) {
				...
				if (extra->data_device[i] != nullptr) {
                    ...
                    */
					ggml_sycl_set_device(i);
					SYCL_CHECK(CHECK_TRY_ERROR(
						sycl::free(extra->data_device[i], *(streams[i]))
					));
				}
			}
			delete extra;
...
GGML_CALL static const char *ggml_backend_sycl_split_buffer_get_name(
	ggml_backend_buffer_t buffer
) {
...
GGML_CALL static void ggml_backend_sycl_split_buffer_free_buffer(
	ggml_backend_buffer_t buffer
) {
	ggml_backend_sycl_split_buffer_context *ctx =
		(ggml_backend_sycl_split_buffer_context *)buffer->context;
	delete ctx;
}

GGML_CALL static void *ggml_backend_sycl_split_buffer_get_base(
	ggml_backend_buffer_t buffer
) {
...
GGML_CALL static void ggml_backend_sycl_split_buffer_init_tensor(
	ggml_backend_buffer_t buffer, ggml_tensor *tensor
) try {
	GGML_ASSERT(
...
GGML_CALL static void ggml_backend_sycl_split_buffer_set_tensor(
	ggml_backend_buffer_t buffer,
	ggml_tensor *tensor,
	const void *data,
	size_t offset,
...
GGML_CALL static void ggml_backend_sycl_split_buffer_get_tensor(
	ggml_backend_buffer_t buffer,
	const ggml_tensor *tensor,
	void *data,
	size_t offset,
...
GGML_CALL static void ggml_backend_sycl_split_buffer_clear(
	ggml_backend_buffer_t buffer, uint8_t value
) {
	UNUSED(buffer);
	UNUSED(value);
...
GGML_CALL static const char *ggml_backend_sycl_split_buffer_type_name(
	ggml_backend_buffer_type_t buft
) {
	return GGML_SYCL_NAME "_Split";
...
GGML_CALL static ggml_backend_buffer_t
ggml_backend_sycl_split_buffer_type_alloc_buffer(
	ggml_backend_buffer_type_t buft, size_t size
) {
	...
	ggml_backend_sycl_split_buffer_context *ctx =
		new ggml_backend_sycl_split_buffer_context();

	return ggml_backend_buffer_init(
		buft, ggml_backend_sycl_split_buffer_interface, ctx, size
	);
}

GGML_CALL static size_t ggml_backend_sycl_split_buffer_type_get_alignment(
	ggml_backend_buffer_type_t buft
) {
	return 128;
	UNUSED(buft);
...
GGML_CALL static size_t ggml_backend_sycl_split_buffer_type_get_alloc_size(
	ggml_backend_buffer_type_t buft, const ggml_tensor *tensor
) {
	ggml_backend_sycl_split_buffer_type_context *ctx =
		(ggml_backend_sycl_split_buffer_type_context *)buft->context;
...
static ggml_backend_buffer_type_i
	ggml_backend_sycl_split_buffer_type_interface = {
...
ggml_backend_sycl_split_buffer_type(const float *tensor_split) {
	static std::mutex mutex;
	std::lock_guard<std::mutex> lock(mutex);
	...
	static std::map<
		std::array<float, GGML_SYCL_MAX_DEVICES>,
	...
	bool all_zero =
		tensor_split == nullptr || std::all_of(
									   tensor_split,
									   tensor_split + GGML_SYCL_MAX_DEVICES,
	...
	struct ggml_backend_buffer_type buft {
		/* .iface   = */ ggml_backend_sycl_split_buffer_type_interface,
			/* .context = */
			new ggml_backend_sycl_split_buffer_type_context{tensor_split_arr},
	};
	...
	static struct ggml_backend_buffer_type ggml_backend_sycl_buffer_type_host =
		{
			/* .iface    = */ {
				/* .get_name         = */
				ggml_backend_sycl_host_buffer_type_name,
				/* .alloc_buffer     = */
				...
				ggml_backend_cpu_buffer_type()
					->iface.get_alignment,
				/* .get_max_size     = */
				NULL, // TODO: return device.maxBufferLength
					  /* .get_alloc_size   = */
				ggml_backend_cpu_buffer_type()
...
ggml_backend_sycl_get_default_buffer_type(ggml_backend_t backend) {
	ggml_backend_sycl_context *sycl_ctx =
		(ggml_backend_sycl_context *)backend->context;
	return ggml_backend_sycl_buffer_type(sycl_ctx->device);
}

GGML_CALL static void ggml_backend_sycl_set_tensor_async(
	...
	GGML_ASSERT(
		buf->buft == ggml_backend_sycl_buffer_type(sycl_ctx->device) &&
...
GGML_CALL static void ggml_backend_sycl_get_tensor_async(
	ggml_backend_t backend,
	const ggml_tensor *tensor,
	void *data,
	size_t offset,
...
GGML_CALL static bool ggml_backend_sycl_cpy_tensor_async(
	ggml_backend_t backend, const ggml_tensor *src, ggml_tensor *dst
) try {
	ggml_backend_sycl_context *sycl_ctx =
		(ggml_backend_sycl_context *)backend->context;
	if (dst->buffer->buft == ggml_backend_sycl_buffer_type(sycl_ctx->device) &&
        ...
        */
		const queue_ptr stream = sycl_ctx->stream(sycl_ctx->device, 0);
		SYCL_CHECK(CHECK_TRY_ERROR(
			(stream)->memcpy(dst->data, src->data, ggml_nbytes(dst)).wait()
		));
		return true;
	}
...
GGML_CALL static bool ggml_backend_sycl_supports_op(
	...
	case GGML_OP_MUL_MAT_ID: {
		...
		if (a_type == GGML_TYPE_IQ4_NL || a_type == GGML_TYPE_IQ4_XS ||
			...
			a_type == GGML_TYPE_IQ2_XXS || a_type == GGML_TYPE_IQ2_XS ||
			a_type == GGML_TYPE_IQ2_S || a_type == GGML_TYPE_IQ1_S ||
			a_type == GGML_TYPE_IQ1_M) {
	...
	case GGML_OP_CONCAT: {
		...
		return ggml_is_contiguous(op->src[0]) &&
			   ggml_is_contiguous(op->src[1]) && src0_type != GGML_TYPE_I32 &&
			   src0_type != GGML_TYPE_I16 && dim == 2;
	} break;
	case GGML_OP_DUP:
	case GGML_OP_NONE:
	case GGML_OP_RESHAPE:
...
GGML_CALL static bool ggml_backend_sycl_offload_op(
	ggml_backend_t backend, const ggml_tensor *op
) {
...
GGML_CALL static bool ggml_backend_sycl_supports_buft(
...
static ggml_backend_i ggml_backend_sycl_interface = {
	...
	/* .cpy_tensor_async        = */
	NULL, //ggml_backend_sycl_cpy_tensor_async, // TODO: update for the new interface
...
bool ggml_backend_is_sycl(ggml_backend_t backend) {
	return backend != NULL &&
		   ggml_guid_matches(backend->guid, ggml_backend_sycl_guid());
}
...
GGML_CALL static ggml_backend_t ggml_backend_reg_sycl_init(
