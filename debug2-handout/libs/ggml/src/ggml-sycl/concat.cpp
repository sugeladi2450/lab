...
static void concat_f32_dim0(
	...
	if (nidx < ne00) { // src0
		int offset_src =
			nidx + item_ct1.get_group(1) * ne00 +
...
static void concat_f32_sycl(
	...
	int num_blocks =
		(ne0 + SYCL_CONCAT_BLOCK_SIZE - 1) / SYCL_CONCAT_BLOCK_SIZE;
	sycl::range<3> gridDim(ne2, ne1, num_blocks);
	switch (dim) {
	case 0:
		stream->parallel_for(
			sycl::nd_range<3>(
				gridDim * sycl::range<3>(1, 1, SYCL_CONCAT_BLOCK_SIZE),
...
static void concat_f32_sycl_non_cont(
	...
	stream->parallel_for(
		...
		[=](sycl::nd_item<3> item_ct1) {
			...
			int64_t o[4] = {0, 0, 0, 0};
			o[dim] =
				dim == 0 ? ne00 : (dim == 1 ? ne01 : (dim == 2 ? ne02 : ne03));
			...
			for (int i0 = item_ct1.get_local_id(2); i0 < ne0;
				...
				if (i0 < ne00 && i1 < ne01 && i2 < ne02 && i3 < ne03) {
					x = (const float *)(src0 + (i3)*nb03 + (i2)*nb02 +
										(i1)*nb01 + (i0)*nb00);
				} else {
					x = (const float *)(src1 + (i3 - o[3]) * nb13 +
										(i2 - o[2]) * nb12 +
										(i1 - o[1]) * nb11 +
										(i0 - o[0]) * nb10);
				}
				...
				float *y =
					(float *)(dst + i3 * nb3 + i2 * nb2 + i1 * nb1 + i0 * nb0);
...
void ggml_sycl_op_concat(
	...
	if (ggml_is_contiguous(src0) && ggml_is_contiguous(src1)) {
		...
		if (dim != 3) {
			...
			SYCL_CHECK(
				CHECK_TRY_ERROR(stream->memcpy(dst_d, src0_d, size0).wait())
			);
