...
#warning "Querying the number of bytes of free memory is not supported"
	...
	sycl::queue create_queue(
		sycl::device device, bool enable_exception_handler = false
	) {
	...
	sycl::queue create_in_order_queue(
		...
		std::lock_guard<mutex_type> lock(m_mutex);
		return create_queue_impl(
			device, enable_exception_handler, sycl::property::queue::in_order()
		);
	}
	...
	void destroy_queue(sycl::queue queue) {
		...
		_queues.erase(
			std::remove_if(
				_queues.begin(),
				_queues.end(),
				[=](const sycl::queue &q) -> bool { return q == queue; }
			),
			_queues.end()
		);
	}
...
private:
	...
	sycl::queue create_queue_impl(
		bool enable_exception_handler, Properties... properties
	) {
...
public:
	...
	inline std::string get_preferred_gpu_platform_name() {
		...
		for (const auto &platform : platform_list) {
			...
			auto gpu_dev = std::find_if(
				devices.begin(),
				devices.end(),
				[](const sycl::device &d) { return d.is_gpu(); }
			);
...
private:
	...
	static int convert_backend_index(std::string &backend) {
		...
		if (backend == "opencl:acc")
			return 5;
		printf(
			"convert_backend_index: can't handle backend=%s\n", backend.c_str()
		);
...
static inline void *dpct_malloc(
	size_t &pitch, size_t x, size_t y, size_t z, sycl::queue &q
) {
...
static inline sycl::event dpct_memset(
	sycl::queue &q, void *dev_ptr, valueT value, size_t size
) {
...
static inline std::vector<sycl::event> dpct_memset(
	sycl::queue &q, pitched_data data, valueT value, sycl::range<3> size
) {
...
template <typename valueT>
static inline std::vector<sycl::event> dpct_memset(
	sycl::queue &q, void *ptr, size_t pitch, valueT val, size_t x, size_t y
) {
	return dpct_memset(
...
static memcpy_direction deduce_memcpy_direction(
	sycl::queue &q, void *to_ptr, const void *from_ptr, memcpy_direction dir
) {
	switch (dir) {
	...
	case memcpy_direction::automatic: {
		...
		return direction_table[static_cast<unsigned>(
			get_pointer_attribute(q, to_ptr)
...
static inline size_t get_copy_range(
	sycl::range<3> size, size_t slice, size_t pitch
) {
...
static inline std::vector<sycl::event> dpct_memcpy(
	...
	public:
		host_buffer(
			size_t size, sycl::queue &q, const std::vector<sycl::event> &deps
		) :
	...
	case device_to_device:
		event_list.push_back(q.submit([&](sycl::handler &cgh) {
			...
			cgh.parallel_for<
				class dpct_memcpy_3d_detail>(size, [=](sycl::id<3> id) {
...
inline unsigned vectorized_binary(
	unsigned a, unsigned b, const BinaryOperation binary_op
) {
...
public:
	using accessor_t =
		typename detail::memory_traits<Memory, T>::template accessor_t<
			Dimension>;
	...
	device_memory(
		...
		for (auto sub_list : init_list) {
			...
			std::memcpy(
				tmp_data, sub_list.begin(), sub_list.size() * sizeof(T)
			);
			tmp_data += in_range[1];
		}
	}
	...
	void init(sycl::queue &q) {
		...
		if (_host_ptr)
			detail::dpct_memcpy(
				q, _device_ptr, _host_ptr, _size, host_to_device
			);
	}
...
inline T atomic_fetch_add(T *addr, T operand, sycl::memory_order memoryOrder) {
	...
	default:
		assert(
			false && "Invalid memory_order for atomics. Valid memory_order for "
...
template <
	...
	typename T1,
	typename T2>
inline T1 atomic_fetch_add(
	T1 *addr, T2 operand, sycl::memory_order memoryOrder
) {
