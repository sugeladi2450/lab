...
static void im2col_kernel(
	...
	for (int64_t i = global_id; i < pelements;
		...
		const int64_t offset_dst = ((batch * OH + oh) * OW + ix) * CHW +
								   (ic * (KW * KH) + ky * KW + kx);
...
static void im2col_sycl(
	...
	const int64_t num_blocks =
		(parallel_elements + SYCL_IM2COL_BLOCK_SIZE - 1) /
		SYCL_IM2COL_BLOCK_SIZE;

	// decrease global range when it exceeds the max int
	int64_t local_size = downsample_sycl_global_range(
		batch * IC * OH * num_blocks, SYCL_IM2COL_BLOCK_SIZE
	);
