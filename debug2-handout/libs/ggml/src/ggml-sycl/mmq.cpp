...
typedef void (*allocate_tiles_sycl_t)(
	int **x_ql, sycl::half2 **x_dm, int **x_qh, int **x_sc
);
...
#pragma unroll
	for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
		...
		const int qh =
			get_int_from_uint8_aligned(bxi->qh, 0) >> (4 * (k % QI5_1));
...
#pragma unroll
	...
	const uint8_t *scales =
		((const uint8_t *)&x_sc[i * (WARP_SIZE / 4) + i / 4 + kbx * 4]) +
		ky / 4;
...
#pragma unroll
	for (int i0 = 0; i0 < mmq_y; i0 += nwarps * 4) {
		...
		const int sc_low =
			(get_int_from_uint8(bxi->scales, ksc_low) >> shift_low) &
			0x0F0F0F0F;
...
static __dpct_inline__ float vec_dot_q3_K_q8_1_mul_mat(
	...
	const int8_t *scales =
		((const int8_t *)(x_sc + i * (WARP_SIZE / 4) + i / 4 + kbx * 4)) +
		ky / 4;
...
#else
		x_dm[i * (WARP_SIZE / QI4_K) + i / QI4_K + kbxd] = {
			bxi->dm[0], bxi->dm[1]
		};
#endif
	}
...
#pragma unroll
		for (int j = 0; j < QI8_1; ++j) {
			sumi_d = dpct::dp4a(
				(v[j] >> (4 * i)) & 0x0F0F0F0F, u[i * QI8_1 + j], sumi_d
			); // SIMD dot product
		}
...
#pragma unroll
	for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
		...
		const int qh0 =
			((qh >> (2 * (kqsx / (QI5_K / 4)) + 0)) << 4) & 0x10101010;
		const int qh1 =
			((qh >> (2 * (kqsx / (QI5_K / 4)) + 1)) << 4) & 0x10101010;
...
#pragma unroll
	for (int i0 = 0; i0 < mmq_y; i0 += nwarps) {
		...
		x_ql[i * (2 * WARP_SIZE + 1) + kq0] =
			dpct::vectorized_binary<sycl::char4>(
