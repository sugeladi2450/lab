...
static void timestep_embedding_f32(
	...
	float freq =
		(float)sycl::native::exp(-(sycl::log((float)max_period)) * j / half);
...
static void timestep_embedding_f32_sycl(
	...
	int half_ceil  = dim / 2;
	int num_blocks = (half_ceil + SYCL_TIMESTEP_EMBEDDING_BLOCK_SIZE - 1) /
					 SYCL_TIMESTEP_EMBEDDING_BLOCK_SIZE;
	sycl::range<3> block_dims(1, 1, SYCL_TIMESTEP_EMBEDDING_BLOCK_SIZE);
	sycl::range<3> gridDim(1, ne00, num_blocks);
	stream->parallel_for(
		sycl::nd_range<3>(gridDim * block_dims, block_dims),
...
void ggml_sycl_op_timestep_embedding(
	...
	timestep_embedding_f32_sycl(
		src0_d, dst_d, src0->ne[0], dst->nb[1], dim, max_period, stream
	);
}
