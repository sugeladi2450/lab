...
void execute_command(
	const std::string &command, std::string &stdout_str, std::string &stderr_str
) {
#ifdef _WIN32
	...
	while (ReadFile(
			   stdout_read, buffer.data(), buffer.size(), &bytes_read, NULL
		   ) &&
		   bytes_read > 0) {
	...
	while (ReadFile(
			   stderr_read, buffer.data(), buffer.size(), &bytes_read, NULL
		   ) &&
		   bytes_read > 0) {
	...
	if (pid == 0) {
		...
		while ((bytes_read =
					read(stdout_pipe[0], buffer.data(), buffer.size())) > 0) {
			stdout_str.append(buffer.data(), bytes_read);
		}

		while ((bytes_read =
					read(stderr_pipe[0], buffer.data(), buffer.size())) > 0) {
			stderr_str.append(buffer.data(), bytes_read);
		}
...
void matmul_shaders(
	std::vector<std::future<void>> &tasks, bool fp16, bool matmul_id
) {
	...
	std::map<std::string, std::string> base_dict = {
		{"FLOAT_TYPE", fp16 ? "float16_t" : "float"}
	};
	...
	for (const auto &tname : type_names) {
		...
		std::string load_vec_a_unaligned =
			(tname == "f32" || tname == "f16") ? "1" : "2";
		// For aligned matmul loads
		std::string load_vec_a =
			(tname == "f32" || tname == "f16") ? load_vec : "2";
		tasks.push_back(std::async(std::launch::async, [=] {
			string_to_spv(
...
void process_shaders(std::vector<std::future<void>> &tasks) {
	std::cout << "ggml_vulkan: Generating and compiling shaders to SPIR-V"
			  << std::endl;
	...
	for (const auto &tname : type_names) {
		// mul mat vec
		std::string data_a_key = "DATA_A_" + to_uppercase(tname);
		std::string shader	   = (string_ends_with(tname, "_k"))
		...
		if (tname != "f16") {
			tasks.push_back(std::async(std::launch::async, [=] {
				string_to_spv(
					...
					"dequant_" + tname + ".comp",
					merge_maps(
						base_dict, {{data_a_key, "1"}, {"D_TYPE", "float16_t"}}
					)
				);
			}));
		}
		...
		if (!string_ends_with(tname, "_k")) {
			shader = (tname == "f32" || tname == "f16") ? "get_rows.comp"
														: "get_rows_quant.comp";
			...
			tasks.push_back(std::async(std::launch::async, [=] {
				string_to_spv(
					...
					shader,
					{{data_a_key, "1"}, {"B_TYPE", "int"}, {"D_TYPE", "float"}}
				);
			}));
		}
	}
	...
	tasks.push_back(std::async(std::launch::async, [] {
		string_to_spv(
			...
			{{"A_TYPE", "float16_t"}, {"B_TYPE", "float"}, {"D_TYPE", "float"}}
		);
	}));
	tasks.push_back(std::async(std::launch::async, [] {
		string_to_spv(
	...
	tasks.push_back(std::async(std::launch::async, [=] {
		string_to_spv(
			...
			"im2col.comp",
			merge_maps(
				base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "float16_t"}}
			)
		);
	}));
...
void write_output_files() {
	...
	for (const auto &pair : shader_fnames) {
		...
		fprintf(
			hdr, "extern unsigned char %s_data[%zu];\n", name.c_str(), size
		);
...
int main(int argc, char **argv) {
	...
	if (args.find("--output-dir") != args.end()) {
		output_dir =
			args["--output-dir"]; // Directory for containing SPIR-V output
	}
	...
	if (args.find("--no-clean") != args.end()) {
		no_clean =
			true; // Keep temporary SPIR-V files in output-dir after build
	}

	if (!directory_exists(input_dir)) {
		std::cerr << "\"" << input_dir
				  << "\" must be a valid directory containing shader sources"
				  << std::endl;
		return EXIT_FAILURE;
	}
