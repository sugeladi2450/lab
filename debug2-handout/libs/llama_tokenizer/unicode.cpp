...
static std::vector<codepoint_flags> unicode_cpt_flags_array() {
	...
	for (size_t i = 1; i < unicode_ranges_flags.size(); ++i) {
		const auto range_ini =
			unicode_ranges_flags[i - 1];				// codepoint_ini, flags
...
static std::vector<size_t> unicode_regex_split_custom_gpt2(
	...
	); // Reserve memory for the approximate size
	...
	for (auto offset : offsets) {
		...
		for (size_t pos = offset_ini; pos < offset_end; /*pos++*/) {
			...
			if (!(flags2.is_whitespace | flags2.is_letter | flags2.is_number) &&
				flags2.as_uint()) {
				pos += (cpt == ' ');
				while (!(flags2.is_whitespace | flags2.is_letter |
						 flags2.is_number) &&
					   flags2.as_uint()) {
					flags2 = _get_flags(++pos);
				}
...
std::vector<uint32_t> unicode_cpts_normalize_nfd(
	...
	for (size_t i = 0; i < cpts.size(); ++i) {
		...
		auto it			   = std::upper_bound(
						 unicode_ranges_nfd.cbegin(),
						 unicode_ranges_nfd.cend(),
						 cpt,
						 comp
					 ) -
				  1;
...
std::vector<std::string> unicode_regex_split(
	...
	if (need_collapse) {
		...
		for (size_t i = 0; i < cpts.size(); ++i) {
			...
			} else if (k_ucat_cpt.find(flags.category_flag()) !=
					   k_ucat_cpt.end()) {
		...
		try {
			...
			if (use_collapsed) {
				...
				for (size_t i = 0; i < regex_expr.size(); ++i) {
					...
					if (regex_expr[i + 0] == '\\' &&
						i + 4 < regex_expr.size() && regex_expr[i + 1] == 'p' &&
						...
						if (k_ucat_enum.find(pat) != k_ucat_enum.end()) {
							...
							regex_expr_collapsed +=
								k_ucat_cpt.at(k_ucat_enum.at(pat));
							regex_expr_collapsed +=
								k_ucat_map.at(k_ucat_enum.at(pat));
							if (!inside) {
								regex_expr_collapsed += ']';
							}
							i += 4;
				...
				const std::wstring wregex_expr =
					unicode_wstring_from_utf8(regex_expr);

				// std::wregex \s does not mach non-ASCII whitespaces, using 0x0B as fallback
				std::wstring wtext(cpts.begin(), cpts.end());
				for (size_t i = 0; i < wtext.size(); ++i) {
					if (wtext[i] > 0x7F &&
						unicode_cpt_flags(wtext[i]).is_whitespace) {
						wtext[i] = 0x0B;
					}
				}
				...
				//printf("regex_expr: %s\n", regex_expr.c_str());
				bpe_offsets =
					unicode_regex_split_stl(wtext, wregex_expr, bpe_offsets);
			}
		} catch (std::regex_error &e) {
			fprintf(
				stderr, "Failed to process regex: '%s'\n", regex_expr.c_str()
			);
